...
<!--- Validação de sequencia para motor --->
  <cfif url.item eq 'MOTOR'>
    <cfset form.cod_veiculo = url.cod_veiculo>
  </cfif>
<!--- --->
...
<cfelseif findnocase("AJ", cookie.cod_item) EQ "4">
  ...
  <cfquery name="buscaItem" datasource="SINCPROD">
    ...
    and cod_item like UPPER('%#cookie.cod_item#%')
  </cfquery>
...
<cfquery name="buscaVeiculo4" datasource="MES">
  ...
  <cfif ListFind(listaDisp, ListaMes4) GT 0 OR ListFind(listaDisp, ListaMes3) GT 0 OR ListFind(listaDisp, ListaMes2) GT 0 OR ListFind(listaDisp, ListaMes) GT 0> 
      ...
      <cfif url.item eq 'MOTOR'>
        ...
        <cfquery datasource="SINCPROD" name="validaSeq">
            SELECT BARCODE1, CAMPO2, CAMPO3, 
            REGEXP_SUBSTR(CAMPO3, '- (\d+)/\d+', 1, 1, NULL, 1) AS SEQ_GRUPO
            FROM INTCOLDFUSION.FILA_ETIQUETAS_CENTRAL 
            WHERE ID_IMP = '25'
            AND BARCODE1 = '#form.cod_veiculo#'
        </cfquery>

        <cfoutput>
        <cfif validaSeq.SEQ_GRUPO neq form.seq_grupo>
            <script>
              alert("A sequência informada é inválida!\nSeq. Informada: #form.seq_grupo#\nSeq. do barcode #validaSeq.BARCODE1#: #validaSeq.CAMPO3#")
    ...
    <!--- --->
      ...
      <cfquery datasource="SINCPROD" name="logs">
        ...
        (USUARIO, VALIDACAO, BARCODE, MODELO, STATUS, DATA, TIPO, PRODUTO) VALUES
        ('#cookie.user#', 'Central: <cfif len(listaDisp) LE 2> VAZIO <cfelse> #listaDisp# </cfif>| MES: #ListaMes# - #ListaMes2# - #ListaMes3# - #ListaMes4#','#form.cod_veiculo#', '#buscaVeiculo.modelo#', 'OK', sysdate, '#url.item#', '#cookie.cod_item#')
      </cfquery>
  ...
  <cfelse>
    <cfquery datasource="SINCPROD" name="logs">
      insert into intcoldfusion.logs_pokayoke
      (USUARIO, VALIDACAO, BARCODE, MODELO, STATUS, DATA, TIPO, PRODUTO) VALUES
      ('#cookie.user#', 'Central: <cfif len(listaDisp) LE 2> VAZIO <cfelse> #listaDisp# </cfif>| MES: #ListaMes# - #ListaMes2# - #ListaMes3# - #ListaMes4#','#form.cod_veiculo#', '#buscaVeiculo.modelo#', 'ERRO', sysdate, '#url.item#', '#cookie.cod_item#') 
    </cfquery>
